...
public class SinglyLinkedList<E> implements Serializable{
	 ...
	 * <p>
	 * Preconditions: p and newnode are not null<\p>
	 * <p>
	 * Postcondition: newnode is inserted after p<\p>
	 * 
	 * @param p
	 *            node preceding new node
	 * @param newnode
	...
	/**
	 * Find the first node containing target, return null if target is not found
	 * 
	 * @param target
	 *            the element to locate
	 * @return the node containing the target element, null if the target was not in
	 *         the list
	 */
	 ...
	 * 
	 * @param p
	 *            the position whose node is to be returned
	 * @return the node at position p
	 * @throws IndexOutOfBoundsException
	 *             if the index p is outside the range 0 to this.length - 1
	 */
